...
export function Summary({items = []}) {
    const totalPrice = items.reduce((prev, curr) => prev + (curr.price * curr.count), 0) //prev로 전달하는 첫 값은 0.
    return (
        <React.Fragment>
            <div>
            ...
            <hr/>
            {items.map(v => <SummaryItem key={v.id} productName={v.productName} count={v.count}/>)}
            <form>
                <div className="mb-3">

...
function App() {
    ...
    const [orders, setOrders] = useState([
        {orderId: '?',email:'?',address: '?',postcode:'?',status:'?'},
    ]);
    ...
    const handleOrdersSubmit = (search) => {
    ...
    const handleDeleteClicked = productId => {
        ...
        const found = items.find(v => v.productId === productId);
        const updatedItems1 =
            found ? items.map(v => (v.productId === productId) && (v.count > 0) ? {...v, count: v.count - 1} : v) : [...items, {...product, count: 0}]
    ...
    return (
        <div className="container-fluid">
            ...
            <ProductSearch orders={orders} onSearchClick={handleOrdersSubmit}/>
